...
class CertificateIssueInfo {
    ...
    /**
     * Validates the JSON data with respect to <code>CertificateIssueInfo</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>CertificateIssueInfo</code>.
     */
    static validateJSON(data) {
        // ensure the json data is a string
        if (data['cert_issuer_type'] && !(typeof data['cert_issuer_type'] === 'string' || data['cert_issuer_type'] instanceof String)) {
            throw new Error("Expected the field `cert_issuer_type` to be a primitive type in the JSON string but got " + data['cert_issuer_type']);
        }
        // validate the optional field `pki_cert_issuer_details`
        if (data['pki_cert_issuer_details']) { // data not null
          PKICertificateIssueDetails.validateJSON(data['pki_cert_issuer_details']);
        }
        // validate the optional field `ssh_cert_issuer_details`
        if (data['ssh_cert_issuer_details']) { // data not null
          SSHCertificateIssueDetails.validateJSON(data['ssh_cert_issuer_details']);
        }
